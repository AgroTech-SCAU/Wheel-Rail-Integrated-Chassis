/**
 * @file fs_ia10b.c
 * @brief FlySky FS-iA10B iBUS 接收机驱动实现
 */
...
/**
 * @brief 将一个接收字节输入 iBUS 帧解析状态机
 */
static void fs_ia10b_process_byte(FsIa10b* self, uint8_t byte,
    ...
    if(self->frame_index == 0u) {
        ...
        if(self->frame_index >= FS_IA10B_IBUS_FRAME_LEN) {
            ...
            received_checksum =
                (uint16_t)(self->frame[30] | ((uint16_t)self->frame[31] << 8u));
...
bool fs_ia10b_is_online(const FsIa10b* self, uint32_t timeout_ms) {
    ...
    return (uint32_t)(self->ops->now_ms() - self->data.last_update_ms) <
           timeout_ms;
}
