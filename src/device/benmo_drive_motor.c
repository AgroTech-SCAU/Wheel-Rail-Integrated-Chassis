...
{
    return motor_id >= BENMO_DRIVE_MOTOR_ID_MIN && motor_id <= BENMO_DRIVE_MOTOR_ID_MAX;
}
...
static void benmo_drive_encode(BenmoDriveMotor *self, uint8_t motor_id, int16_t rpm)
{
    int32_t scaled;
    uint8_t index;
                                             ...
                                             const BenmoDriveMotorConfig *config)
{
    if (self == NULL || config == NULL || config->ops == NULL ||
        ...
        config->ops->critical_enter == NULL || config->ops->critical_exit == NULL ||
...
BenmoDriveMotorStatus benmo_drive_motor_request_next_feedback(BenmoDriveMotor *self)
{
    ...
    data[3] = 5U;
    data[4] = 0xAAU;
    if (!self->ops->send_standard(BENMO_DRIVE_MOTOR_QUERY_CAN_ID,
                                  data,
                                  BENMO_DRIVE_MOTOR_CAN_DATA_LEN))
    {
        self->tx_error_count++;
                                                        ...
                                                        uint8_t len)
{
    ...
    feedback = &self->nodes[self->last_query_id - BENMO_DRIVE_MOTOR_ID_MIN].feedback;
...
BenmoDriveMotorStatus benmo_drive_motor_check_feedback(const BenmoDriveMotor *self)
{
    ...
    for (i = 0U; i < BENMO_DRIVE_MOTOR_COUNT; ++i)
    {
        ...
        uint32_t reference = feedback->valid ? feedback->last_update_ms : self->initialized_at_ms;
