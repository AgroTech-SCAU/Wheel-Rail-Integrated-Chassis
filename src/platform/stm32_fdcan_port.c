...
static Stm32FdcanCallbackSlot g_rx_slots[STM32_FDCAN_BUS_COUNT];
...
static bool stm32_fdcan_send(Stm32FdcanBus bus,
                             uint32_t id,
                             ...
                             uint32_t timeout_ms)
{
    ...
    while (HAL_FDCAN_GetTxFifoFreeLevel(handle) == 0U) {
        if (timeout_ms == 0U || (uint32_t)(HAL_GetTick() - start_ms) >= timeout_ms) {
            return false;
        }
    }
    memset(&header, 0, sizeof(header));
    header.Identifier = id;
    header.IdType = id_type == STM32_FDCAN_ID_EXTENDED
                        ? FDCAN_EXTENDED_ID
                        : FDCAN_STANDARD_ID;
    ...
    return HAL_FDCAN_AddMessageToTxFifoQ(handle, &header, (uint8_t*)data) == HAL_OK;
}
...
bool stm32_fdcan_port_send_standard(Stm32FdcanBus bus,
                                    ...
                                    uint32_t timeout_ms)
{
    return stm32_fdcan_send(bus, id, STM32_FDCAN_ID_STANDARD, data, len, timeout_ms);
}
...
bool stm32_fdcan_port_send_extended(Stm32FdcanBus bus,
                                    ...
                                    uint32_t timeout_ms)
{
    return stm32_fdcan_send(bus, id, STM32_FDCAN_ID_EXTENDED, data, len, timeout_ms);
}
                                           ...
                                           void* context)
{
    ...
    if (g_rx_slots[bus].callback != NULL &&
        (g_rx_slots[bus].callback != callback || g_rx_slots[bus].context != context)) {
        return false;
    }
    g_rx_slots[bus].callback = callback;
    g_rx_slots[bus].context = context;
    return true;
}
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef* handle, uint32_t interrupts)
{
    ...
    while (HAL_FDCAN_GetRxFifoFillLevel(handle, FDCAN_RX_FIFO0) > 0U) {
        ...
        if (HAL_FDCAN_GetRxMessage(handle, FDCAN_RX_FIFO0, &header, data) != HAL_OK) {
            return;
        }
        ...
        if (g_rx_slots[bus].callback != NULL) {
            g_rx_slots[bus].callback(&frame, g_rx_slots[bus].context);
        }
    }
}
